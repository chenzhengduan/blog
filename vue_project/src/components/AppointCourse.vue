...
const pixelToTime = (x, y) => {
  ...
  const hourDecimal = y / 120          // 每小时120px，从0:00开始
...
const handleMouseDown = (event, course) => {
  ...
  if (positionInfo) {
    ...
    finalStartY = courseStart * 120  // 每小时120px
...
const handleMouseMove = (event) => {
  ...
  dragState.value.animationFrameId = requestAnimationFrame(() => {
    ...
    // Y轴边界：限制在时间范围内（0:00-24:00，共24小时，每小时120px）
    const totalGridHeight = 24 * 120  // 24小时 * 120px/小时
...
const handleMouseUp = (event) => {
  ...
  const alignedY = hour * 120  // 修改：直接使用hour，不减去6
...
const calculateDayLayout = (dayIndex) => {
  ...
  sortedCourses.forEach(course => {
    ...
    layouts.set(course.id, {
      ...
      top: `${courseStart * 120}px`, // 修改：直接使用courseStart，不减去6
      ...
      <div class="time-list">
        ...
        <div class="time-labels">
          ...
          <div 
            ...
            :style="{ top: `${(parseTime(dragState.currentTime)) * 120}px` }"
          >
