...
- 约束：
  ...
  * 检测时间为投放时间后一周
- 输出：当前目录生成 zhb_survival_data.json，数组JSON，key为中文字段名
*/
...
// 配置参数
const START_DATE = new Date('2024-05-20');
const END_DATE = new Date('2025-07-25');
// 生成上限（520-580之间随机）
const MAX_RECORDS = 520 + Math.floor(Math.random() * 61);
...
function generateRecord(pondId, stockDate) {
  ...
  const record = {
    ...
    '综合存活率（%）': 0 // 占位，稍后用记录字段计算，确保严格等式
  };

  // 以记录字段直接计算综合存活率，确保：综合存活率 = 基础存活率 × 环境修正因子
...
function main() {
  ...
  const outPath = './zhb_survival_data.json';
