    ...
    <div class="calendar-container" :style="{ '--week-days-count': weekDays.length }" v-loading="loading" element-loading-target=".calendar-container">
      ...
      <table class="period-calendar-table">
          ...
          <tr>
            ...
            <th class="week-header sticky">
              <div class="flex-center">
                ...
                >
                  <template #reference>
                    <el-icon size="16px" color="var(--wtwo-color-primary)" class="ml-2px cursor-pointer setting-btn" title="日期设置">
            ...
            <th
              ...
              v-if="props.timetableType !== CourseTimetableTypeEnum.TimeTimetable"
            >
              <div class="header-cell-box">
                <div class="teacher-header-name ellipsis-single w-123px" :title="t.name">{{ t.name }}</div>
                <div class="teacher-course-num-header">{{ getTeacherCourseCount(t.id) }}条</div>
                <div
            ...
            <th 
              ...
              class="view-more-header sticky"
              :class="{ 'fixed-width': props.assign==1 }"
            >
              <el-button
                type="primary"
                plain
            ...
            <th 
              v-if="props.assign==1"
              class="add-object-header sticky"
            >
              <div class="flex-center cursor-pointer" @click="handleObjectSelectClick">
                <el-icon v-if="props.assign==1" size="16px" class="cursor-pointer setting-btn" :title="'添加'+transToConfigDescript(objectName)">
                  <svg aria-hidden="true">
            ...
            <!-- 占位列，填充剩余空间（仅在没有添加列和查看更多列时显示） -->
            <th 
              v-if="!(props.assign==1) && !(showViewMore&&visibleTeachers.length>0&&!(props.assign==1 && (!assignList || assignList.length === 0)))"
        ...
        <tbody>
          ...
          <tr v-if="timetableType === CourseTimetableTypeEnum.StudentTimetable && (!assignList || assignList.length === 0)">
            <td :colspan="2 + visibleTeachers.length + (props.assign==1 ? 1 : 0) + (showViewMore&&visibleTeachers.length>0&&!(props.assign==1 && (!assignList || assignList.length === 0)) ? 1 : 0) + (!(props.assign==1) && !(showViewMore&&visibleTeachers.length>0&&!(props.assign==1 && (!assignList || assignList.length === 0))) ? 1 : 0)" class="empty-state">
          ...
          <!-- 正常内容行 -->
          <template v-else>
            <!-- 每一天（周一到周日） - 每天按时段拆分成多行 -->
            <template v-for="(day, dayIdx) in weekDays" :key="day.key">
              <tr v-for="(period, periodIdx) in timePeriods" :key="`${day.key}-${period.key}`">
                <!-- 星期信息（仅在第一个时段行显示，使用rowspan合并） -->
                <td v-if="periodIdx === 0" class="week-cell" :rowspan="timePeriods.length">
                  <div class="week-cell-content">
                    <div class="week-name flex-center" :class="{ today: day.isToday }">
                        {{ day.weekText }}
                        <div class="today-circle" v-if="day.isToday">今</div>
                    </div>
                    <div class="week-date">
                      {{ day.dateText }}
                    </div>
                    <span v-if="day.isHoliday" class="holiday-flag-inline">假</span>
                  </div>
                </td>
                <!-- 时段标识 -->
                ...
                <td 
                  v-if="showViewMore&&visibleTeachers.length>0&&!(props.assign==1 && (!assignList || assignList.length === 0))"
                  class="view-more-cell"
                >
                  
                </td>
                <!-- 添加对象列（如果是指定模式） - 当前时段的添加列 -->
                <td 
                  v-if="props.assign==1"
                  class="add-object-cell"
                >
      ...
      <el-dialog v-model="overLimitTipVisible" title="温馨提示" width="420px">
        <div class="py-16px line-height-22px">
          当前课表显示的{{ transToConfigDescript(objectName) }}数量过多，可能会影响体验，建议切换到“时刻”视图
        </div>
        <template #footer>
...
async function loadData(force = false) {
  ...
  return new Promise<void>((resolve, reject) => {
    // force 为 true 时立即执行，否则延迟 300ms（防抖）
...
// 监听属性变化 - 监听周、搜索参数和 assignList 的变化
watch(
  [() => props.currentWeek, () => props.searchParams, () => props.viewBy, () => props.assignList, () => props.assign],
  async (nv, ov) => {
    ...
    const viewByChanged = ov ? nv[2] !== ov[2] : false
    const assignListChanged = ov ? nv[3] !== ov[3] : false
    const assignChanged = ov ? nv[4] !== ov[4] : false

    // 如果有任何变化，重新加载数据
    if (weekChanged || paramsChanged || viewByChanged || assignListChanged || assignChanged) {
      await loadData()
...
const updateColumnWidth = () => {
  // 对象列已设置为固定宽度160px，不需要动态计算
  // 此函数保留为空函数，避免其他地方调用时报错
}
...
// 组件挂载时加载数据和设置列宽
// 注意：由于 watch 已经会在初始化时触发（如果有数据），这里不需要再次调用 loadData
// 但为了确保首次加载，还是保留，依靠 loadData 内部的防抖和去重机制避免重复
onMounted(async () => {
  // watch 已经设置了 immediate: true，会在组件挂载时自动加载数据
  // 这里只需要初始化列宽即可
  nextTick(() => {
    updateColumnWidth()
  })
