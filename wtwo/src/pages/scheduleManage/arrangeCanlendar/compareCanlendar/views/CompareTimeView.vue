<template>
  <div class="w100% h100% flex canlendar-container flex-col is-compare"
    :class="{ 'is-fullscreen': isFullscreen }"
    v-loading="loading || refreshing" element-loading-target=".canlendar-container">
        ...
        <div
          ...
          <template v-if="props.timetableType === CourseTimetableTypeEnum.StudentTimetable">
            <div class="more-btn ml-4px mt-1px" @click.stop="openStudentPopover($event, props.assignList[0])"
              v-click-outside="(event) => handleStudentClickOutside(event, props.assignList[0])">
    ...
    <div class="schedule-container is-compare" ref="scheduleContainerRef">
      <!-- 表头行 -->
      <div class="header">
        <!-- 时间列（对比模式固定45px宽） -->
        <div class="time-column-simple is-compare">
          时刻
        </div>
        <div v-for="(day, index) in props.weekDays" :key="index" class="day-column" :class="{
          'day-column-highlight': dragState.isDragging && dragState.targetDay === index,
        }">
          <div class="day-column-content">
            <div class="day-name" :class="{ 'date-today': isToday(weekDates[index]) }">{{ day }}</div>
            <div v-if="isToday(weekDates[index])" class="day-date-today">今</div>
            <div class="day-date" v-else>
              {{  '' }}
            </div>
            <!-- 节假日标记在对比模式下不显示 -->
            <!-- 更多课程标记 -->
            <div v-if="daysWithHiddenCourses[index]" @click="openMoreDrawer(weekDates[index])" class="more-courses-mark"
              title="该天有更多排课未显示">
              更多
            </div>
          </div>
      ...
      <div class="time-schedule-content">
        ...
        <div class="time-list">
          ...
          <div class="schedule-grid"
               ...
               @dragleave="handleScheduleDragLeave"
               @drop="handleScheduleDrop"
               :class="{ 'drag-over': isDragOver && props.timetableType === CourseTimetableTypeEnum.TeacherTimetable }">
            <div v-for="(day, dayIndex) in weekDays" :key="dayIndex" class="day-column-container">
              <!-- 网格背景 -->
              <div class="grid-background" @click="handleContainerClick($event, dayIndex)">
                <div v-for="slot in timeSlots" :key="slot.value" class="grid-cell" :class="{
                  'past-date': isBeforeToday(weekDates[dayIndex])
                }" :style="{
                  height: (slot.isLast ? 0 : defaultHeight / 2) + 'px',
                }"></div>
              ...
              <!-- 课程容器 - 绝对定位 -->
              <div class="courses-container" @click="handleContainerClick($event, dayIndex)">
                <div v-for="course in coursesByDay[dayIndex].courses"
                  :key="course.ID" class="course-item" :class="{
                    ...
                    'short-course': false && parseFloat(getCourseStyle(course, dayIndex, getStatusColor(course)).height) < 90,
                    'no-permission': course.HasCampusPermission === false
                  }" :style="getCourseStyle(course, dayIndex, getStatusColor(course))"
                  :title="`${course.ShiftName}\n${course.StartTime} - ${course.EndTime}`"
                  v-click-outside="(event) => handleClickOutside(event, item)"
                  ...
                  <!-- 新增：上下边缘拖拽手柄 - 🆕 只有有权限的排课才显示 -->
                  <div v-if="course.HasCampusPermission !== false" class="resize-handle resize-handle-top"
                  ...
                  <div v-if="course.HasCampusPermission !== false" class="resize-handle resize-handle-bottom"
                ...
                <div v-for="course in coursesByDay[dayIndex].schedules"
                  :key="course.ID" class="course-item" :class="{
                    'resizing': dragState.isResizing && dragState.draggedCourse?.ID === course.ID,
                    'no-permission': course.HasCampusPermission === false
                  }" :style="getCourseStyle(course, dayIndex, '#2877FF')"
                ...
                <!-- 临时占位块 - 只在对应的天显示 -->
                <div v-if="temporaryBlock && temporaryBlock.day === dayIndex" class="temporary-block"
                ...
                <!-- 🆕 拖拽排课 - 目标格子高亮 -->
                <div v-if="dragTargetCell && dragTargetCell.day === dayIndex" 
                     class="drag-target-cell"
                     :style="{
                       top: dragTargetCell.top + 'px',
                       height: dragTargetCell.height + 'px'
                     }">
                  <div class="drag-target-cell-content">
                    <div class="drag-target-time">
                      {{ hourToTimeString(dragTargetCell.startHour) }} - {{ hourToTimeString(dragTargetCell.endHour) }}
                    </div>
...
const props = defineProps({
  ...
  // ✅ 已删除的 props（对比模式下永远是固定值）:
  // - searchParams: 不需要，数据从 externalData 传入
  // - assign: 对比模式永远是 1（指定对象）
  // - isCompare: 对比模式永远是 true
  // - viewBy: 对比模式永远是 'byTime'
})

// ✅ 对比模式固定值
const minWidth = 150  // 对比模式固定最小宽度

const defaultHeight = computed(() => {
  return 30 * 2;  // 对比模式固定行高
});
...
const getCurrentObjectCourseCount = computed(() => {
  // 统计 internalCourses 中类型为 'course' 的课程数量
  return internalCourses.value.filter(item => item.type === 'course').length
})
...
const updateCoursesFromSelectedObject = () => {
  ...
  if (selectedObjectData && (selectedObjectData.CourseList || selectedObjectData.ScheduleList)) {
    ...
    const courseList = Array.isArray(selectedObjectData.CourseList)
      ? selectedObjectData.CourseList.map((it, idx) => normalizeItem(it, 'course', idx))
      : []
    const scheduleList = Array.isArray(selectedObjectData.ScheduleList)
      ? selectedObjectData.ScheduleList.map((it, idx) => normalizeItem(it, 'schedule', idx))
      : []
...
function processExternalData() {
  ...
  try {
    ...
    // 通知父组件课程数量变化
    const courseCount = internalCourses.value.filter(item => item.type === 'course').length
    emit('course-count-update', courseCount)
  } catch (error) {
...
async function loadCourseData(forceRefresh = false) {
  ...
  // 以下是非对比模式的逻辑
  console.error('❌❌❌ CompareTimeView loadCourseData 被调用了！这不应该发生！', {
    ...
    loadCourseDataTimer = setTimeout(async () => {
      ...
      try {
        ...
        if (currentRequestId === pendingRequestId) {
          if (props.timetableType === CourseTimetableTypeEnum.TimeTimetable) {
            // 时间视图数据处理
            internalCourses.value = (res.Data?.List || []).map((item, index) => ({
              ...item,
              day: getDayOfWeek(dayjs(item.StartTime).format('YYYY-MM-DD'), dayjs(props.currentWeek).startOf('week').format('YYYY-MM-DD')),
              StartTime: dayjs(item.StartTime).format('HH:mm'),
              EndTime: dayjs(item.EndTime).format('HH:mm'),
...
// 监听当前周变化，更新时间线显示
// ✅ 对比模式：监听周次和外部数据变化
watch(() => [props.currentWeek, props.externalData], (newVal, oldVal) => {
  // 对比模式：只处理外部数据，不调用API
  if (props.externalData) {
    processExternalData()
  }
...
const dragTargetCell = ref(null) // 当前拖拽悬停的目标格子 { day, hour, top, height }
...
// 🆕 拖拽排课 - 开始拖拽学员
const handleDragScheduleStart = (event) => {
...
// 🆕 拖拽排课 - 拖拽结束
const handleDragScheduleEnd = (event) => {
  // 拖拽结束后的清理工作
  // 🆕 恢复鼠标样式
...
const handleScheduleDragOver = (event) => {
  ...
  // 🚀 性能优化：使用 RAF + 节流批处理更新
  const scheduleGrid = event.currentTarget
  const clientX = event.clientX
  const clientY = event.clientY
  
  // 缓存边界矩形（只在第一次或容器变化时计算）
  ...
  dragOptimization.rafId = requestAnimationFrame(() => {
    ...
    const { clientX, clientY, rect } = dragOptimization.pendingUpdate
    const relativeX = clientX - rect.left
    const relativeY = clientY - rect.top
    
    // 🆕 使用精确位置模式（不计算中心点偏移）
    const dropPosition = pixelToTime(relativeX, relativeY, true)
    if (dropPosition && dropPosition.day >= 0 && dropPosition.day <= 6) {
      const startHourValue = dropPosition.hour
      const endHourValue = Math.min(startHourValue + 0.5, endHour.value) // 默认0.5小时（30分钟）
      
      // 计算格子的视觉位置（与临时占位块保持一致：上下各留2px间距）
      const top = (startHourValue - startHour.value) * defaultHeight.value + 2
      const height = (endHourValue - startHourValue) * defaultHeight.value - 4
...
// 🆕 拖拽排课 - 离开老师课表
const handleScheduleDragLeave = (event) => {
  if (props.timetableType !== CourseTimetableTypeEnum.TeacherTimetable) return
  // 🆕 恢复鼠标样式
  document.body.style.cursor = ''
...
// 🆕 拖拽排课 - 放置到老师课表
const handleScheduleDrop = (event) => {
  if (props.timetableType !== CourseTimetableTypeEnum.TeacherTimetable) return
  event.preventDefault()
  // 🆕 恢复鼠标样式
  document.body.style.cursor = ''
  isDragOver.value = false
  dragTargetCell.value = null // 清理高亮状态
  ...
  try {
    ...
    const dropPosition = pixelToTime(relativeX, relativeY, true)
    if (!dropPosition || dropPosition.day < 0 || dropPosition.day > 6) {
      ElMessage.warning('请拖拽到有效的日期位置')
      return
    }
    
    const targetDate = weekDates.value[dropPosition.day]
    const startHourValue = dropPosition.hour
    const targetTime = hourToTimeString(startHourValue)
    
    // 计算结束时间（默认0.5小时，即30分钟）
    const endHourValue = Math.min(startHourValue + 0.5, endHour.value) // 默认0.5小时（30分钟），不超过当天结束时间
    const endTime = hourToTimeString(endHourValue)
...
// 处理课程容器的点击事件（空白区域点击）
const handleContainerClick = (event, dayIndex) => {
  // 检查点击是否在课程块上
  ...
  // console.log('创建临时占位块（精确时间段）:', {
  //   天: props.weekDays[dayIndex],
  //   点击Y坐标: Math.round(relativeY),
  //   点击时间: hourDecimal.toFixed(2),
  //   所在30分钟段: halfHourIndex,
  //   计算开始时间: blockStartHour.toFixed(2),
  //   计算结束时间: blockEndHour.toFixed(2),
  //   限制后开始时间: clampedStartHour.toFixed(2),
  //   限制后结束时间: clampedEndHour.toFixed(2),
  //   最终开始时间: finalStartHour.toFixed(2),
  //   最终结束时间: finalEndHour.toFixed(2),
  //   占位块时间段: `${StartTime} - ${EndTime}`,
  //   top位置: `${parseTime(StartTime) * defaultHeight.value}px`,
  //   网格对齐验证: `每30分钟30px，与${defaultHeight.value / 2}px匹配 ✓`,
  //   边界检查: `确保${StartTime} < ${EndTime} 或者 ${EndTime} = 00:00`,
  //   说明: '占位块显示在点击位置所在的30分钟时间段，完整24小时范围：00:00-24:00'
  // })
}
...
const addSchedule = () => {
  ...
  if (props.timetableType === CourseTimetableTypeEnum.TimeTimetable) {
    ...
    // ✅ 对比模式：从 assignList 获取对象信息（学员/老师）
    const assignObject = props.assignList && props.assignList[0]
    const campusId = (assignObject && (assignObject.CampusID || assignObject.CampusList)) || '';
    ...
    if (assignObject) {
      scheduleData = {
        ...
        object: {
          ...
          avatar: assignObject.Avatar || assignObject.Photo || assignObject.Image,
          campusId: campusId
        }
      }
      // 除了老师课表，其他的都有校区id
  ...
  // ✅ 对比模式：不清除临时占位块，保持显示以便用户查看
  // temporaryBlock.value = null  // 主视图会清除，对比模式保留
...
const pixelToTime = (x, y, useDirectPosition = false) => {
  const dayWidth = getDayColumnWidth(); // 动态获取列宽

  let dayIndex;
  
  // 🆕 拖拽排课时使用直接位置，不计算中心点偏移
  if (useDirectPosition) {
    ...
    // 基础的天数计算 - 使用中心点位置
    dayIndex = Math.floor(centerX / dayWidth)
    ...
    if (dragState.value.isDragging && dragState.value.draggedCourse) {
      ...
      if (Math.abs(centerX - originalCenterX) < 30 && Math.floor(centerX / dayWidth) === originalDay) {
...
const handleCourseMouseDown = (event, course) => {
  // 🆕 检查是否有校区权限，如果没有权限则不允许拖拽
  if (course.HasCampusPermission === false) {
    return
  }
...
const handleMouseUp = (event) => {
  ...
  // 检查是否发生了实际移动
  const hasPositionChanged = course.day !== day || course.StartTime !== newStartTime
...
const handleRefreshData = () => {
  ...
  if (props.externalData) {
    ...
    if (props.assignList && props.assignList[0]) {
      ...
      console.warn('对比模式下无法刷新：assignList 为空')
      // 降级方案：直接处理现有数据（虽然是旧数据）
      processExternalData()
    }
  } else {
    // 非对比模式：直接重新加载课程数据
...
const getDragPreviewStyle = (color) => {
  ...
  if (dragState.value.isDragging) {
    ...
    // 调整高度：使用记录的精确列位置
    const dayWidth = getDayColumnWidth();
    const dayLeft = course.day * dayWidth;
    currentLeft = dayLeft + dragState.value.startPosition.courseX; // 应用记录的相对left
...
const getDragPreviewTimeStyle = () => {
  ...
  if (dragState.value.isDragging) {
    ...
    // 调整高度：使用原始位置
    const dayWidth = getDayColumnWidth();
    currentLeft = dragState.value.draggedCourse.day * dayWidth + dragState.value.startPosition.courseX;
...
const handleResizeStart = (event, course, direction) => {
  ...
  if (courseElement) {
    ...
    dragState.value.originalWidth = rect.width
    dragState.value.originalHeight = rect.height
    dragState.value.startPosition.courseX = rect.left - parentRect.left // 记录相对父元素的left
  }
...
/* 🆕 老师课表拖拽悬停效果（整体提示，已被精确格子高亮取代，可选保留） */
...
/* 🆕 拖拽目标格子高亮样式 - 与临时占位块完全一致 */
.drag-target-cell {
  position: absolute !important;
  left: 4px !important;
  width: calc(100% - 8px) !important; /* 左右各留4px间距 */
  background-color: rgba(240, 242, 245, 0.75) !important;
  border-radius: 4px !important;
  backdrop-filter: blur(2px) !important;
  box-sizing: border-box !important;
  ...
  /* 使用 outline 实现更小间距的虚线（2px 实线 2px 间隙） */
