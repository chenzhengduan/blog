<template>
  <div class="w100% h100% flex canlendar-container"
    ...
    <div v-if="showObjectColumn" class="object-column-container">
      ...
      <div class="header" style="border-top-left-radius: 8px;">
        <!-- 对象列（当显示对象列时） -->
        <div class="object-column-header">
      ...
      <div class="time-schedule-content  object-column-content">
        <!-- 对象列（当显示对象列时） -->
        <div v-if="showObjectColumn" class="object-column-wrapper">
          <div class="object-list" ref="objectListRef">
            <div v-for="object in objects" :key="object.ID" class="object-item"
              :class="{ 'selected': selectedObject?.ID === object.ID }" @click="handleObjectSelect(object)">
              <div class="object-info">
                <div class="object-name" :title="object.Name">{{ object.Name }}</div>
                <div v-if="object.CourseCount !== undefined" class="flex-center mt-3px">
                  <div class="object-count">{{ object.CourseCount }}条</div>
                  <div v-if="props.timetableType == CourseTimetableTypeEnum.StudentTimetable"
                    class="student-more-btn ml-4px mt-1px" @click.stop="openStudentPopover($event, object)"
                    v-click-outside="(event) => handleStudentClickOutside(event, object)">
                    <el-icon class="default-icon" size="12px" color="#606266"><svg aria-hidden="true">
                        <use xlink:href="#w2-xueyuanxinxi"></use>
                      </svg></el-icon>
                    <el-icon class="active-icon" size="12px" color="#fff"><svg aria-hidden="true">
                        <use xlink:href="#w2-xueyuanxinxi"></use>
                      </svg></el-icon>
                  </div>
                  <el-dropdown trigger="click" v-if="props.assign==1" :show-timeout="0" :hide-timeout="0" @click.stop>
                    <div class="more-btn ml-2px mt-1px" @click.stop>
    ...
    <div class="schedule-container" ref="scheduleContainerRef">
      ...
      <div class="header">
        ...
        <div v-for="dayData in filteredWeekData" :key="dayData.originalIndex" class="day-column" :class="{
          'day-column-highlight': dragState.isDragging && dragState.targetDay === dayData.originalIndex,

        }">
          <!-- 'day-column-today': isToday(dayData.date) -->
          <div class="day-column-content">
            <div class="day-name" :class="{ 'date-today': isToday(dayData.date) }">{{ dayData.dayName }}</div>
            <div v-if="isToday(dayData.date)" class="day-date-today">今</div>
            <div class="day-date" v-else>
              {{ formatDate(dayData.date) }}
            </div>
            <!-- 假标记 -->
            <div v-if="isHoliday(dayData.date)" class="holiday-mark"
              :title="getHolidayInfo(dayData.date)?.Name || '节假日'">
              假
            </div>
            <!-- 更多课程标记 -->
            <div v-if="daysWithHiddenCourses[dayData.originalIndex]" @click="openMoreDrawer(dayData.date)" class="more-courses-mark"
              title="该天有更多排课未显示">
              更多
            </div>
          </div>
          ...
          <!-- 拖拽时显示目标指示器 -->
          <!-- <div 
          v-if="dragState.isDragging && dragState.targetDay === dayData.originalIndex" 
        ...
        <!-- 日期选择列 -->
        <div class="weekday-select-column">
      ...
      <div v-else class="time-schedule-content">
        ...
        <div class="time-list">
          ...
          <div class="schedule-grid">
            <div v-for="dayData in filteredWeekData" :key="dayData.originalIndex" class="day-column-container">
              <!-- 网格背景 -->
              <div class="grid-background" @click="handleContainerClick($event, dayData.originalIndex)">
                <div v-for="slot in timeSlots" :key="slot.value" class="grid-cell" :class="{
                  'past-date': isBeforeToday(dayData.date)
                }" :style="{
                  height: (slot.isLast ? 0 : defaultHeight / 2) + 'px',
                }"></div>
              ...
              <div class="courses-container" @click="handleContainerClick($event, dayData.originalIndex)">
                <div v-for="course in coursesByDay[dayData.originalIndex].courses"
                  :key="course.ID" class="course-item" :class="{
                    'resizing': dragState.isResizing && dragState.draggedCourse?.ID === course.ID,
                    'short-course': false && parseFloat(getCourseStyle(course, dayData.originalIndex, getStatusColor(course)).height) < 90
                  }" :style="getCourseStyle(course, dayData.originalIndex, getStatusColor(course))"
                ...
                <div v-for="course in coursesByDay[dayData.originalIndex].schedules"
                  :key="course.ID" class="course-item" :class="{
                    'resizing': dragState.isResizing && dragState.draggedCourse?.ID === course.ID
                  }" :style="getCourseStyle(course, dayData.originalIndex, '#2877FF')"
                ...
                <!-- 临时占位块 - 只在对应的天显示 -->
                <div v-if="temporaryBlock && temporaryBlock.day === dayData.originalIndex" class="temporary-block"
          ...
          <!-- 日期选择占位列 - 与表头对齐，不可操作 -->
          <div class="weekday-select-placeholder"></div>
...
const addSchedule = () => {
  ...
  if (props.timetableType === CourseTimetableTypeEnum.TimeTimetable) {
    ...
    const campusId = (selectedObject.value && (selectedObject.value.CampusID || selectedObject.value.CampusList)) || '';
    // 对象+时间视图：需要添加对象信息
    if (selectedObject.value) {
      scheduleData = {
        ...
        object: {
          ...
          avatar: selectedObject.value.Avatar,
          campusId: campusId
        }
      }
      // 除了老师课表，其他的都有校区id
...
const getDragPreviewStyle = (color) => {
  ...
  if (dragState.value.isDragging) {
    ...
    // 找到原始天在过滤后列表中的索引
    const filteredDayIndex = filteredWeekData.value.findIndex(d => d.originalIndex === course.day)
    const displayDayIndex = filteredDayIndex !== -1 ? filteredDayIndex : 0
    
    const dayLeft = displayDayIndex * dayWidth;
...
/* 日期选择占位列 - 与表头对齐 */
.weekday-select-placeholder {
  width: 40px;
  min-width: 40px;
  flex-shrink: 0;
  background: #fff;
  border-right: 1px solid #e8e8e8; // 添加右边框，与表头一致
  pointer-events: none; // 不可操作
}
