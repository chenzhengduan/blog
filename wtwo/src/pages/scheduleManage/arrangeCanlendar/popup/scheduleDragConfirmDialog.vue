    ...
    <el-dialog v-model="dialogVisible" :show-close="true" width="640px" :close-on-click-modal="false"
      ...
      <div v-if="confirmInfo" class="dialog-content">
        <div class="course-info">
          <div class="course-title">{{ confirmInfo.course.Title }}</div>
          <div class="info-grid">
            <div class="info-row">
              <span class="info-label">参与员工：</span>
              <span class="info-value" :title="formatTeacherList(confirmInfo.course.TeacherList)">{{
                formatTeacherList(confirmInfo.course.TeacherList) }}</span>
            </div>
            <div class="info-row">
              <span class="info-label">{{transToConfigDescript('所属校区：')}}</span>
              <span class="info-value" :title="confirmInfo.course.CampusName">{{ confirmInfo.course.CampusName || '-'
                }}</span>
            </div>
            <div class="info-row">
              <span class="info-label">日程类型：</span>
              <span class="info-value" :title="confirmInfo.course.Content">{{ confirmInfo.course.Content || '-'
                }}</span>
            </div>
            <div class="info-row">
              <span class="info-label">日程备注：</span>
              <span class="info-value" :title="confirmInfo.course.Remark">{{ confirmInfo.course.Remark || '-' }}</span>
            </div>
          </div>
        </div>
        <pageAttentionTips class="ml-8px! mb-30px! mt-8px!">仅调整“<span
            class="color-#FF7D00">当前日程</span>”，若该批次创建了多个日程，则其他日程不会更改！
        </pageAttentionTips>
        ...
        <div class="time-adjust">
          ...
          <div class="time-section new">
            ...
            <div class="time-header"></div>
            <div class="time-row">
              <span class="time-label" :class="{ 'value-changed': isDateChanged }">日程日期：</span>
            ...
            <div class="time-row">
              <span class="time-label" :class="{ 'value-changed': isTimeChanged }">日程时间：</span>
              <div class="time-range-container">
                <el-time-picker :clearable="false" v-model="confirmInfo.newPosition.StartTime" size="small"
                  ...
                  :class="['custom-time-picker', { 'value-changed': isTimeChanged }]" @change="handleStartTimeChange" />
                ...
                <el-time-picker :clearable="false" v-model="confirmInfo.newPosition.EndTime" size="small"
                  ...
                  :class="['custom-time-picker', { 'value-changed': isTimeChanged }]" @change="handleEndTimeChange" />
              </div>
              <!-- <el-icon class="edit-icon">
                <Edit />
              </el-icon> -->
...
import { CopyOrMoveSchedule } from '@/api/arrange';
...
// 格式化教师列表
const formatTeacherList = (teacherList) => {
...
const getChangeSummary = computed(() => {
  ...
  if (details.teacherChanged) {
    changes.push(`老师: ${details.teacherChange}`)
  }
...
function handleConfirm() {
  ...
  CopyOrMoveSchedule({
    ScheduleList: [{
      ID: confirmInfo.value.course.ID,
      DateList: [{
    ...
    ElMessage.success("日程" + handlerName + "成功。");
  ...
  .time-adjust {
    ...
    .time-section {
      ...
      .time-row {
        ...
        :deep(.custom-date-picker.wtwo-date-editor.wtwo-input__wrapper) {
