...
export default class FetchService {
  ...
  // get接口没有请求体,键值对转化为参数
  async get<T = any>(params: IRequestParams,options:RequestInit={}): Promise<IResponse<T>> {
    let url =
  ...
  async post<T = any>(params: IRequestParams,options:RequestInit={}): Promise<IResponse<T>> {
    return this._request("POST", params ,options);
  }

  async put<T = any>(params: IRequestParams,options:RequestInit={}): Promise<IResponse<T>> {
    return this._request("PUT", params ,options);
  }

  async delete<T = any>(params: IRequestParams ,options:RequestInit={}): Promise<IResponse<T>> {
    return this._request("DELETE", params ,options);
  }
