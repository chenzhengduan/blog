...
用于在 3D 场景中显示一个箭头，通常用于可视化方向向量，帮助调试坐标方向、法线、速度向量等。 ![](/_next/static/media/helper-1.44d94841.png)
...
红色代表 X 轴. 绿色代表 Y 轴. 蓝色代表 Z 轴. ![](/_next/static/media/helper-2.3149983c.png)
...
`BoxHelper` 用于**在 3D 场景中可视化物体的包围盒 (Bounding Box)**，通常用于调试和检测物体的边界。

![](/_next/static/media/helper-3.e1e2e36d.png)
...
`Box3Helper` 用于**可视化 `Box3`（轴对齐包围盒，AABB，Axis-Aligned Bounding Box）**。  
通常用于**调试模型边界**、**碰撞检测**、**计算物体包围范围**等场景。

**`BoxHelper`** 适用于**单个 `Mesh` 或 `Group`**，会自动计算物体的包围盒，但不会用于碰撞检测；而 **`Box3Helper`** 适用于**手动定义 `Box3` 包围盒**，可以计算多个对象的整体边界，并支持碰撞检测。若需简单可视化 `Mesh` 的边界，用 `BoxHelper`；若需手动设定范围或合并多个物体的边界，用 `Box3Helper`。

![](/_next/static/media/helper-4.617d837e.png)
...
`CameraHelper` 会在 `3D` 场景中**绘制摄像机的视锥体**，用来表示摄像机的**视野范围**和**近平面、远平面**等信息。  
**适用于：**
...
![](/_next/static/media/helper-5.d78d50d4.png)
